...
test("problem response becomes a typed API error", async () => {
  ...
  await assert.rejects(client.commandOutcome("c1", { deadline: new Date("2026-07-22T12:00:00Z") }), (error: unknown) => error instanceof ApiProblem && error.problem.status === 403);
});
