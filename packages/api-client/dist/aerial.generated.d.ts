...
export interface AerialSafetyProvenance {
    source_time: string;
    expires_at: string;
...
export interface AerialSafetyView<T = unknown> {
    api_version: "v1";
    scope: {
        tenant: string;
        region: string;
    ...
    provenance: AerialSafetyProvenance;
    value: T;
}
...
export interface AerialOperatorCommand {
    id: string;
    scope: {
        tenant: string;
        region: string;
    ...
    resource_id: string;
    action: string;
    authority_lane: AerialAuthorityLane;
...
export interface AerialTransport {
    request<T>(path: string, init: RequestInit): Promise<T>;
}
export declare class GeneratedAerialClient {
    private readonly transport;
    constructor(transport: AerialTransport);
    view<T>(incident: string, kind: AerialViewKind, resourceId: string, signal?: AbortSignal): Promise<AerialSafetyView<T>>;
    command(value: AerialOperatorCommand, idempotencyKey: string, signal?: AbortSignal): Promise<{
        command_id: string;
        outcome: AerialActionOutcome;
    }>;
}
